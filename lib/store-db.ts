...
export function addOrder(order: Omit<Order, "id" | "createdAt" | "status">): Order {
  ...
  (async () => {
    try {
      const isFree = newOrder.pricingType === "free" || (newOrder.finalPrice || 0) === 0;

      // Always log free claims to free_downloads table if applicable
      if (isFree && newOrder.customerEmail && newOrder.productId) {
        try {
          await supabase.from("free_downloads").insert([
            {
              email: newOrder.customerEmail,
              product_id: newOrder.productId.length === 36 ? newOrder.productId : undefined,
      ...
      const payload: any = {
        ...
        final_price: newOrder.finalPrice !== undefined && newOrder.finalPrice !== null ? newOrder.finalPrice : (newOrder.productPrice || 0),
        payment_method: newOrder.paymentMethod || "instapay",
        payment_screenshot: newOrder.screenshotUrl || "",
        status: newOrder.status || "pending",
        product_slug: newOrder.productId || newOrder.productTitle,
        discount_code: newOrder.couponCode || null,
        created_at: newOrder.createdAt,
      };

      if (newOrder.productId && newOrder.productId.length === 36) {
